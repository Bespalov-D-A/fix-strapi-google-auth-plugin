...
module.exports = ({ strapi }) => ({
      ...
      try {
        ...
        let scopesObject = null
           ...
           scopesObject = JSON.parse(scopesData);
              ...
              console.log(`Change to parcing error: ${e}. I continue the execution of the program without parsing`)
            }
  ...
  getUserProfile(code) {
    console.log('adfadfasfgasgsfafas')
    return new Promise(async (resolve, reject) => {
      try {
        let credentials = await this.getGoogleCredentials();
        if (!credentials) {
        ...
        if (!user) {
          ...
          return resolve({
            data: {
              token: strapi.service('plugin::users-permissions.jwt').issue(newUser),
        ...
        resolve({
          data: {
            token: strapi.service('plugin::users-permissions.jwt').issue(user),
